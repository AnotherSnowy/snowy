{
  ...
  let
    ...
    flakeIdentifier =
      ...
      then throw "An attempt was made to access attribute ${attrPath} on a value that's supposed to be a flake, but may not be a proper flake."
      else
...
in {
  ...
  config = {
      ...
      mapAttrs (
        attrName: attrConfig:
          flake.${
            ...
            system
          } or (throw (perInputAttributeError {
            inherit
              system
              flake
              attrName
    ...
    perSystem = {
      options = mapAttrs (_k: _v: lib.mkOption {}) (filterAttrs (_k: v: v.adHoc) config.transposition);
    };
  };
}
