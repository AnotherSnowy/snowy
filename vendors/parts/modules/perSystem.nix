  ...
  options = {
    ...
    perSystem = mkOption {
        ...
        {system, ...}: {
          ...
          config = {
            _module.args = {
                ...
                mapAttrs (
                  inputName: input:
                    ...
                    (
                      ...
                      else throw "Trying to retrieve system-dependent attributes for input ${escapeNixIdentifier inputName}, but this input is not a flake. Perhaps flake = false was added to the input declarations by mistake, or you meant to use a different input, or you meant to use plain old inputs, not inputs'."
                    )
                )
                self.inputs;
              ...
              self = throwAliasError' "self";
              inputs = throwAliasError' "inputs";
              getSystem = throwAliasError "getSystem";
              withSystem = throwAliasError "withSystem";
              moduleWithSystem = throwAliasError "moduleWithSystem";
            };
          };
        }
      );
      apply = modules: system:
        (lib.evalModules {
          modules = lib.lists.toList modules;
          prefix = [
            "perSystem"
            system
          ];
