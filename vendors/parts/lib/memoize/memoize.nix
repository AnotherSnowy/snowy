{lib, ...}: let
  keys = let
    nonNullBytesStr = builtins.readFile ./bytes.dat;
    nonNullItems = lib.stringToCharacters nonNullBytesStr;
  ...
  */
  makeTrie = prefix: f:
    lib.mapAttrs (k: v:
      if k == ""
      then f prefix
      else makeTrie (prefix + k) f)
    keys;

  queryTrie = trie: needle: let
    needleList = lib.stringToCharacters needle;
    destination = lib.foldl' (subtrie: c: subtrie.${c}) trie needleList;
  in
    destination."";
in {
  /**
